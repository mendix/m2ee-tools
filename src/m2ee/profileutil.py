...
class Log:
  def __init__(self, request_id, data): 
    ...
    if hasattr(self, "start_time"):
        ...
        self.start_time_formatted = datetime.datetime.fromtimestamp(self.start_time//1000)
  ...
  def pretty_format(self, print_queries=True):
    if print_queries:
      queries = "\n\n".join(["query: %s \nduration:%s" % (x['query'], x['duration']) for x in self.queries])
...
Form: %s \n \
Original request: %s \n\n \
" % (queries, self.request_id, self.username, userroles, self.still_running, self.action, self.start_time_formatted, self.end_time_formatted, self.duration, form_name, self.request_content)
...
def print_logs(logs):
    ...
    column_names_in_order = ['', 'action', 'duration', 'start_time_formatted', 'end_time_formatted', 'username', 'still_running']
    print format_dict_table(ordered_logs[:50], max_column_width=width/columns+200, column_names=column_names_in_order)
