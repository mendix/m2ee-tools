...
def print_all(client, options, name, config=False):
    ...
    try:
        ...
        if config:
            ...
            return # stop here, no output of values
...
graph_info This graph shows the amount of user requests this MxRuntime handles""" % name
...
%s.type DERIVE
%s.min 0""" % (substrip,subname,substrip,substrip,subname,substrip,substrip)
...
graph_category Mendix
graph_info This graph shows the amount of started transactions and executed database queries
""" % name
...
# named_user_sessions counts names as well as anonymous
# sessions
...
graph_title %s - MxRuntime Users
graph_category Mendix
graph_info This graph shows the amount of user accounts and concurrent sessions""" % name
...
max.draw LINE1
max.info Total maximum size of memory that could be allocated for this JVM""" % name
...
max_threads_and_queue.info Maximum number of threads and queue size
""" % name
